...
for c in corners:
    cv2.circle(img, tuple(c[0].astype(np.uint)), 10, (0, 255, 0), 2)

for c in img_pts.squeeze().astype(np.float32):
    cv2.circle(img, tuple(c.astype(np.uint)), 5, (0, 0, 255), 2)
...
for c in img_pts.squeeze().astype(np.float32):
    cv2.circle(img, tuple(c.astype(np.uint)), 2, (255, 255, 0), 2)
